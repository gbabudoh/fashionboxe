  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        toast.success('ACCESS GRANTED. ENTERING THE RUNWAY.', {
          style: {
             background: '#000',
             color: '#d4af37',
      ...
      toast.error('SYSTEM ERROR. BOOT ABORTED.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full opacity-20 pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-fuchsia/20 blur-[120px] rounded-full" />
      ...
      >
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-black text-white tracking-tight uppercase">Concession Access</h1>
          <p className="mt-4 text-[10px] font-black uppercase tracking-[0.4em] text-white/40">Secure Identity Core v1.0</p>
        </div>
        ...
        <div className="bg-white/[0.03] backdrop-blur-3xl rounded-[3rem] border border-white/10 p-10 shadow-2xl relative">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                <Mail className="absolute left-6 top-1/2 -translate-y-1/2 h-4 w-4 text-white/20 group-focus-within:text-accent transition-colors" />
                <input 
                  type="email" 
                  value={email}
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <Lock className="absolute left-6 top-1/2 -translate-y-1/2 h-4 w-4 text-white/20 group-focus-within:text-fuchsia transition-colors" />
                <input 
                  type="password" 
                  value={password}
          ...
          <div className="mt-10 pt-8 border-t border-white/5 flex flex-col items-center gap-4">
             <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">
               Trouble accessing the runway?
             </p>
             <button className="text-[10px] font-black text-accent uppercase tracking-widest hover:text-white transition-colors">
               Contact Concession Support
             </button>
          </div>
        </div>
        
        <p className="mt-8 text-center text-[9px] font-medium text-white/10 uppercase tracking-[0.2em]">
          © 2026 Fashionboxe Concessions. Proprietary Infrastructure.
        </p>
      </motion.div>
    </div>
  );
